# Avatarian — developer guide

The full walkthrough of the tool: how it's built, deployed, and how the
glyphs are designed. For a quick orientation start at `README.md`; for the
*script itself* see `AVATARIAN.md`.
...
> question, gathered in one place. This guide covers the *tool*: how it's
> built and deployed.
...
corpus/          <- the attested corpus: words somebody has SEEN written
  attested.json    edit this (or use the workbench)
...
flush (9-row blocks). These proportional heights are always on — they are
the base CSS rules; there is no toggle or `avatarian-proportional` class.
...
and whether a vowel is **3-row or 4-row**. Both live in the design, so the
designer — where you actually find these out — records them directly:
...
So `ə` is drawn once, in its top-slot form, and the bottom form is derived.
`ɜ` does not exist anywhere in the pipeline, and `er`, `ur` and `ir` are
accepted spellings of `uh`.
...
neither is in `FLIPS`. Both are read off the corpus and live in `render.js`:
...
* **One unassigned mark in the key.** An unlabelled wedge sits directly
  ...
  `null_v` (the ∪ cup) and `null_c` (a squared ∪) are the two null fillers,
  and **neither is a sound**. A null takes the height that meets its
  partner — `null_c` beside a vowel, `null_v` beside a consonant. The
  writing sample shows a null under "not", "mad", "when", "wake" and "but",
  every one of which has an odd phoneme count.

## The English → IPA converter (g2p.js + lexicon.js)

Four layers, in order:

1. **`js/corpus.js`** — the attested corpus: words somebody has actually
   *seen* written in Avatarian. It differs from the three below it in
   kind, not just in priority. They answer "what does this word sound
   like" and let the pairing rule work out the blocks; a corpus entry is
   the finished block structure, nulls and all. That is how `appa` gets
   drawn as three blocks where pairing predicts two, and how `students`
   and `metalbending` get their mid-word nulls. Edit
   `corpus/attested.json` and run `tools/build_corpus.py`.
2. **`EXCEPTIONS`** in `g2p.js` — the hand list. Avatar vocabulary and the
   coined compounds no dictionary has (`waterbender`, `Kyoshi`,
   `Omashu`). Every line of it is a *reading* from English spelling and
   provisional until checked — two of the first four audited were wrong.
   A word that gets confirmed moves up to the corpus rather than being
   corrected here.
3. **`js/lexicon.js`** — the CMU Pronouncing Dictionary, ~126k words,
   mapped onto this project's phoneme set. Generated by
   `tools/build_lexicon.py`; see that file for the mapping decisions.
4. **`RULES`** — the original letter-to-sound scanner, now the fallback
   for anything the first three don't cover.

`lookupWord()` reports which layer answered — `attested`, `derived` or
`guessed` — and `sentenceToIPA` carries it onto every word, so the page
knows how much to trust each spelling it draws. Nothing displays it yet.
...
- G2P is rule-based, not dictionary-grade: unusual words and names may need
  a hand-edit in the Sounds box or an `EXCEPTIONS` entry.
- /x/ has no glyph — no source shows one, so it renders as a dashed box.
- The apostrophe is stripped, not rendered.
