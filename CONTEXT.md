...
9. ~~**G2P accuracy.**~~ **Done** — `js/lexicon.js` bundles the CMU
   Pronouncing Dictionary (~126k words) ahead of the rules, and
   unstressed-vowel reduction now falls out of CMU's stress marks
   (unstressed `AH` → schwa). Order is `EXCEPTIONS` → lexicon → `RULES`.
   ~1.6 MB inline, ~650 KB gzipped, index built lazily on first use.
   Generated by `tools/build_lexicon.py`; the raw dictionary is not
   committed. **The wiki gadget deliberately does NOT load it** — that
   much weight on every article isn't worth it, and `ipa=` covers the
   wiki case. Coined Avatar compounds still live in `EXCEPTIONS`, since
   no general dictionary has "metalbending".
