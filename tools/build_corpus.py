...
This script is also the validator, and it is strict on purpose — a corpus
...
def check_spelling(spelling, symbols, where, errors):
...
def main():
    ...
    symbols = set(json.loads(MANIFEST.read_text(encoding="utf-8")))
    ...
    for entry in data["entries"]:
        ...
        tokens = check_spelling(entry["spelling"], symbols, where, errors)
        ...
        if entry["source"] not in sources:
            errors.append(f"{where}: source '{entry['source']}' is not in sources")

        record = {"ipa": tokens, "source": entry["source"], "confidence": confidence}
        if entry.get("gloss"):
    ...
    if errors:
        print(f"{SRC.relative_to(ROOT)} has {len(errors)} problem(s):", file=sys.stderr)
        for e in errors:
            print("  " + e, file=sys.stderr)
        return 1

    body = json.dumps(
        {"sources": sources, "words": out}, ensure_ascii=False, indent=2
    )
    DST.write_text(f"{HEADER}window.AVATARIAN_CORPUS = {body};\n", encoding="utf-8")

    phrases = sum(1 for k in out if " " in k)
    unsure = sum(1 for v in out.values() if v["confidence"] != "certain")
    print(f"Wrote {DST.relative_to(ROOT)} — {len(out)} entries "
          f"({phrases} phrase(s), {unsure} not certain) "
          f"from {len(sources)} source(s), {DST.stat().st_size // 1024} KB")
    return 0
